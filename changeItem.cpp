...
void changeItem::initChangeItem(const char* fileName) {
    ...
    if (!file.is_open()) {
        // If the file could not be opened, clear the error state and try to create the file
        ...
        file.open(fileName, std::ios::in | std::ios::out | std::ios::binary); // Reopen in read/write mode
    }
